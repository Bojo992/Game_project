...
local counter = 0
local enemySootingTime = math.random(30, 60)

function shoot(event)
    if (event.phase == "began") and (counter >= enemySootingTime - 10) then
...
function shootEnemy()
    if (counter == enemySootingTime) then
        sign = display.newRect(300, 300, 100, 10)
    end

    if (counter == enemySootingTime+120) then
        display.remove(sign)
    end
    
    if (counter == enemySootingTime+60) and (score == 0) then
...
local function onLocalCollision( event )
    if ( event.phase == "began" ) then
        ...
        if ((obj1.myName == "bullet" and obj2.myName == "antaganist") or 
            (obj1.myName == "bulletEnemy" and obj2.myName == "protagonist") or
            (obj1.myName == "antaganist" and obj2.myName == "bullet") or 
