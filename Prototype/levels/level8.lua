...
local antaganist = display.newImage("Sprites\\Enemies\\Revolver Richolas\\Revolver_Richolas_Idle.png", 540, display.contentCenterY+100, 20, 20)
...
--Controls enemy's shooting and sign    
    function shootEnemy()
        ...
        if (counter == closingWindowForShot) and (score == 0) then
            ...
            antaganist = display.newImage("Sprites\\Enemies\\Revolver Richolas\\Revolver_Richolas_Shoot.png", 540, display.contentCenterY+100, 20, 20)
            physics.addBody(antaganist, "static", {radius = 20, isSensor=true })
            antaganist.myName = "antaganist"
    ...
    if (event.phase == "began") then
        ...
        then
            ...
            then
                ...
                display.remove(antaganist)
                local path = "Sprites\\Enemies\\Revolver Richolas\\Revolver_Richolas_"..lifes.."Health.png"
                antaganist = display.newImage(path, 540, display.contentCenterY+100, 20, 20)
                antaganist.myName = "antaganist"
                display.remove(sign)
                counter = 0
